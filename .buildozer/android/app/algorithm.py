import cv2
import numpy as np
from main import CameraClick
from PIL import Image, ImageDraw, ImageFont

list2 = []
list3 = []

lower_red = np.array([30, 1, 1], dtype="uint8")
upper_red = np.array([100, 50, 60], dtype="uint8")
lower_green = np.array([10, 100, 10], dtype="uint8")
upper_green = np.array([30, 254, 30], dtype="uint8")

def detectShape(cnt, number):
    """Ищем квадраты."""
    shape = 'unknown'
    peri = cv2.arcLength(cnt, True)
    vertices = cv2.approxPolyDP(cnt, 0.02 * peri, True)
    sides = len(vertices)
    if(sides == 4):
        shape = 'off'
        list2.append(number)
    return shape

def sorter(lists):
    for i in range(len(lists)-1):
        for j in range((len(lists)-1)-i):
            if lists[j][1] > lists[j+1][1]:
                lists[j], lists[j+1] = lists[j+1], lists[j]
            elif lists[j][1] == lists[j+1][1]:
                if lists[j][0] > lists[j+1][0]:
                    lists[j], lists[j+1] = lists[j+1], lists[j]
    for kl in lists:
        list3.append(kl[2])
    return(list3)



def switcher_recognizer(lower, upper, number):
    list1 = []
    img = CameraClick.capture()
    rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    mask = cv2.inRange(rgb, lower, upper)
    # блюрим изображение (картинка, (размер ядра(матрицы), стандартное отклонение ядра))
    blurred = cv2.GaussianBlur(mask, (1, 1), 0)
    # преобразуем картинку в чб, всем значениям >200 присваиваем 255, остальным-0
    # threshold возвращает два значения - второй переданный в функцию аргумент и картинку
    T, returned_image = cv2.threshold(blurred, 200, 255, cv2.THRESH_BINARY)
    cv2.imshow('matches', img)
    cv2.waitKey(0)
    cv2.imshow('ret', returned_image)
    cv2.waitKey(0)
    contours, hierarchy = cv2.findContours(returned_image, cv2.RETR_EXTERNAL , cv2.CHAIN_APPROX_SIMPLE)

    for cnt in contours:
        moment = cv2.moments(cnt)
        cx = int(moment['m10'] / moment['m00'])
        cy = int(moment['m01'] / moment['m00'])
        shape = detectShape(cnt, 1)
        cv2.drawContours(img, [cnt], -1, (0, 255, 0), 2)
        cv2.putText(img, shape, (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0,0),2)
        cv2.imshow('polygons_detected', img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    for cn in range(0, len(contours)):
        x, y, w, h = cv2.boundingRect(contours[cn])
        if 1 < w and 1 < h:
            u = (x, y, list2[cn])
            list1.append(u)
    return(list1, number)
condition = True
switchers_list, number = switcher_recognizer(lower_red, upper_red, 1)
switchers_list_green, number = switcher_recognizer(lower_green, upper_green, 0)
switchers_list.append(switchers_list_green)
if len(switchers_list) != 8:
    condition = True
else:
    print(list3)
while condition:
    switchers_list = switcher_recognizer()
    if len(switchers_list) != 8:
        condition = True
    else:
        list4 = sorter(list3)
        print(list4)
        condition = False

if len(switchers_list) == 8:
    draw_image = Image.open("right_switch_recognizer/scheme.png")
    font = ImageFont.truetype("right_switch_recognizer/arial.ttf", size=20)
    draw_text = ImageDraw.Draw(draw_image)
    if switchers_list[0] == 1:
        draw_text.text(
            (20, 150),
            "Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[0] == 0:
        draw_text.text(
            (20, 150),
            "Выключен",
            font=font,
            fill=("#19ff19")
        )
    if switchers_list[1] == 1:
        draw_text.text(
            (400, 520),
            "Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[1] == 0:
        draw_text.text(
            (400, 520),
            "Выключен",
            font=font,
            fill=("#19ff19")
        )
    if switchers_list[2] == 1:
        draw_text.text(
            (650, 580),
            "Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[2] == 0:
        draw_text.text(
            (650, 580),
            "Выключен",
            font=font,
            fill=("#19ff19")
        )
    if switchers_list[3] == 1:
        draw_text.text(
            (850, 580),
            "4 Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[3] == 0:
        draw_text.text(
            (850, 580),
            "4 Выключен",
            font=font,
            fill=("#19ff19")
        )
    if switchers_list[4] == 1:
        draw_text.text(
            (850, 555),
            "5 Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[4] == 0:
        draw_text.text(
            (850, 555),
            "5 Выключен",
            font=font,
            fill=("#19ff19")
        )
    if switchers_list[5] == 1:
        draw_text.text(
            (850, 530),
            "6 Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[5] == 0:
        draw_text.text(
            (850, 530),
            "6 Выключен",
            font=font,
            fill=("#19ff19")
        )
    if switchers_list[6] == 1:
        draw_text.text(
            (1142, 530),
            "Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[6] == 0:
        draw_text.text(
            (1142, 530),
            "Выключен",
            font=font,
            fill=("#19ff19")
        )
    if switchers_list[7] == 1:
        draw_text.text(
            (130, 860),
            "Включен",
            font=font,
            fill=("#ff0000")
        )
    elif switchers_list[7] == 0:
        draw_text.text(
            (130, 860),
            "Выключен",
            font=font,
            fill=("#19ff19")
        )
